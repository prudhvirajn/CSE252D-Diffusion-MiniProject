...
from utils import slerp, image_grid
...
class ImageGen:
  def __init__(self, gen_model_id = "stabilityai/stable-diffusion-2-base", interp_model_id="kakaobrain/karlo-v1-alpha-image-variations", interpolate_steps = 5):
    ...
    self.scheduler = EulerDiscreteScheduler.from_pretrained(self.gen_model_id, subfolder="scheduler", steps=5)
    self.stable_diffusion_pipe = StableDiffusionPipeline.from_pretrained(self.gen_model_id, scheduler=self.scheduler, torch_dtype=torch.float16)
    self.stable_diffusion_pipe.to(self.device)
    
    print("Loading Image Interpolation Pipeline")
    #UnCLIP Image Interpolation
    self.interp_model_id = interp_model_id
    self.interpolate_pipe = UnCLIPImageInterpolationPipeline.from_pretrained(self.interp_model_id, torch_dtype = torch.float16)
    self.interpolate_generator = torch.Generator(device='cuda')
    self.interpolate_pipe.to(self.device)
  ...
  """
  Generate sequence of images based on text prompts to form a story
  """
  def generate_sequence(self, prompts, negative_prompt):
    images = []
    for p in prompts:
      images.append(self._generate_image(p, negative_prompt))
    return images

  """
  Helper function to interpolate between two images
  """
  def _interpolate_images(self, im0, im1):
    images = [im0, im1]
    out = self.interpolate_pipe(image = images, generator = self.interpolate_generator, steps=self.interpolate_steps)
    interpolated_images = [out[i] for i in range(len(out))]
    return interpolated_images

  """
  For all story points, generate interpolated steps between them
  """
  def interpolate_story(self, story_scenes):
    story_complete = []
    story_complete.append(story_scenes[0])
    for i in range(len(story_scenes)-1):
      first = story_scenes[i]
      second = story_scenes[i+1]

      result = self._interpolate_images(first, second)
      result = result[0]
      story_complete = story_complete + result

      story_complete.append(story_scenes[i+1])
